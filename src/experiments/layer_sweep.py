...
def run_layer_sweep():
    ...
    for layer_idx in target_layers:
        ...
        # (We need to hack the global LAYER_ID in intervention utils or pass it)
        # For simplicity, we re-implement the vector extraction locally or modify the function
        # Let's assume we copy-paste the logic briefly here for the sweep:
        
        # ... (Vector Extraction Logic for specific layer) ...
        # [Simplified for brevity: Assume we get 'vec' for 'layer_idx']
        
        # Let's use the existing intervention logic but modify the hook dynamically
        ...
        def hook(module, input, output):
            ...
            # Use your strong coefficient
            intervention = -5.0 * vec
            if hidden.dtype == torch.float16: intervention = intervention.half()
            else: intervention = intervention.bfloat16()
            return (hidden + intervention,) + (output[1:] if isinstance(output, tuple) else ())
        ...
        for _, row in tqdm(df.iterrows(), total=len(df), leave=False):
            ...
            with torch.no_grad():
                out = model.generate(inputs.input_ids, max_new_tokens=60, pad_token_id=tokenizer.eos_token_id)
...
def get_steering_vector_for_layer(model, tokenizer, layer_id):
    # This is a stripped down version of your intervention.py logic
    # adapted to accept layer_id
    steering_data = [
